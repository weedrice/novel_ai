...
export default function ScenesPage() {
  ...
  const fetchEpisodes = async () => {
    ...
    try {
      ...
      if (data.length > 0) {
        ...
        fetchScenesByEpisode(data[0].id);
      }
    } catch (err: any) {
  ...
  const fetchScenesByEpisode = async (episodeId: number) => {
    ...
    setError(null);
    try {
      const response = await fetch(`${API}/scenes/episode/${episodeId}`);
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
      const data = await response.json();
      setScenes(data);
    } catch (err: any) {
      setError(`장면 불러오기 실패: ${err.message}`);
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="max-w-6xl mx-auto">
          ...
          <ErrorMessage
            ...
            onRetry={() => {
              ...
              if (selectedEpisodeId) {
                fetchScenesByEpisode(selectedEpisodeId);
              } else {
                fetchEpisodes();
              }
            }}
        ...
        {!loading && scenes.length === 0 && selectedEpisodeId && (
          <Card className="text-center py-12">
            ...
            <p className="text-gray-500 text-lg mb-2">이 에피소드에는 장면이 없습니다</p>
            <p className="text-gray-400 text-sm">새로운 장면을 추가해보세요</p>
          </Card>
        )}
      </div>
    </main>
