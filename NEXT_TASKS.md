...
- [ ] RelationshipService 클래스 생성
- [ ] 관계 CRUD 메서드 구현
- [ ] 특정 캐릭터의 모든 관계 조회 메서드
- [ ] 양방향 관계 처리 로직
...
- [ ] RelationshipController 생성
- [ ] GET /characters/graph 엔드포인트 구현
  - 노드(캐릭터) 목록 반환
  - 엣지(관계) 목록 반환
  - 그래프 라이브러리 호환 형식으로 변환
- [ ] GET /characters/{id}/relationships 엔드포인트 구현
  - 특정 캐릭터의 관계만 조회
- [ ] POST /relationships 엔드포인트 구현
  - 새 관계 생성
- [ ] DELETE /relationships/{id} 엔드포인트 구현
  - 관계 삭제
...
- [ ] 그래프 라이브러리 조사 (react-flow, cytoscape.js, vis.js 등)
- [ ] 선택한 라이브러리 설치
- [ ] 기본 예제 테스트
...
- [ ] 시나리오 생성 요청 메서드 구현
  - generateScenario(sceneId): LLM에 전체 대화 생성 요청
...
- [ ] SceneController에 엔드포인트 추가
  - POST /scenes/{sceneId}/generate-scenario
  - 요청: 장면 ID
  - 응답: 생성된 대화 목록 (화자, 대사, 순서)
- [ ] LLM 서버에 시나리오 생성 엔드포인트 추가
  - POST /gen/scenario
  - 여러 캐릭터가 참여하는 대화 생성
  - 대화 흐름이 자연스럽도록 프롬프트 설계
